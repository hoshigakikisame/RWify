...
<body class="relative bg-white dark:bg-gray-900">
    ...
    <main class="flex">
        @php
            $footerMenu = [
                'Profile' => [
                    ...
                    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-4">
        ...
        <x-sidebar.sidebar-wrap email="{{ auth()->user()->getEmail() }}" role="{{ auth()->user()->getRole() }}"
            :imageProfile="auth()->user()->getImageUrl()" :footerMenu="$footerMenu">
            @yield('menu')
    ...
    <script type="module">
        ...
        document.addEventListener('click', (event) => {
            if (
                !footerMenuButton[0].contains(event.target) &&
                !footerMenuButton[1].contains(event.target) &&
        ...
        $('.overflow-data').on('mouseleave', (e) => {
            ...
            $(e.currentTarget).removeClass(
                'absolute -top-1 text-xs rounded-lg border dark:border-gray-200 z-10 p-2',
            );
        });
    </script>
</body>
