...
@section('menu')
<x-sidebar.sidebar-item href="{{ route('warga.dashboard') }}" title="Dashboard" :active="request()->routeIs(['warga.dashboard'])">
...
<div class="relative" x-data="{sideDropdown: false}">
    <button class="{{$activeDropIuran ? $activeStyle['active']:$activeStyle['default'];}}" @click="sideDropdown = !sideDropdown" x-effect="document.location.pathname.split('/').includes('pembayaranIuran') ? sideDropdown = true : sideDropdown = false">
    ...
    <ul x-show="sideDropdown" class="ms-3 mt-2 flex flex-col gap-2">
        <x-sidebar.sidebar-item href="{{ route('warga.layanan.pembayaranIuran.riwayatPembayaranIuran') }}" title="Riwayat" :active="request()->routeIs(['warga.layanan.pembayaranIuran.riwayatPembayaranIuran'])">
            <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" style="enable-background:new 0 0 512 512" xml:space="preserve" class="lg:w-5 w-6 fill-inherit">
                <path fill-rule="evenodd" d="M20.41 9.86a3 3 0 0 0-.175-.003H17.8c-1.992 0-3.698 1.581-3.698 3.643s1.706 3.643 3.699 3.643h2.433q.092.001.175-.004a1.7 1.7 0 0 0 1.586-1.581c.004-.059.004-.122.004-.18v-3.756c0-.058 0-.121-.004-.18a1.7 1.7 0 0 0-1.585-1.581m-2.823 4.611c.513 0 .93-.434.93-.971s-.417-.971-.93-.971-.929.434-.929.971.416.971.93.971" clip-rule="evenodd" />
                <path fill-rule="evenodd" d="M20.234 18.6a.214.214 0 0 1 .214.27c-.194.692-.501 1.282-.994 1.778-.721.727-1.636 1.05-2.766 1.203-1.098.149-2.5.149-4.272.149h-2.037c-1.771 0-3.174 0-4.272-.149-1.13-.153-2.045-.476-2.766-1.203C2.62 19.923 2.3 19 2.148 17.862 2 16.754 2 15.34 2 13.555v-.11c0-1.785 0-3.2.148-4.306C2.3 8 2.62 7.08 3.34 6.351c.721-.726 1.636-1.05 2.766-1.202C7.205 5 8.608 5 10.379 5h2.037c1.771 0 3.174 0 4.272.149 1.13.153 2.045.476 2.766 1.202.493.497.8 1.087.994 1.78a.214.214 0 0 1-.214.269h-2.433c-2.734 0-5.143 2.177-5.143 5.1s2.41 5.1 5.144 5.1zM5.614 8.886a.725.725 0 0 0-.722.728c0 .403.323.729.722.729H9.47c.4 0 .723-.326.723-.729a.726.726 0 0 0-.723-.728z" clip-rule="evenodd" />
                <path d="m7.777 4.024 1.958-1.443a2.97 2.97 0 0 1 3.53 0l1.969 1.451C14.41 4 13.49 4 12.483 4h-2.17c-.922 0-1.769 0-2.536.024" />
            </svg>
        </x-sidebar.sidebar-item>
        <x-sidebar.sidebar-item href="{{ route('warga.layanan.pembayaranIuran.new') }}" title="Bayar Iuran" :active="request()->routeIs(['warga.layanan.pembayaranIuran.new'])">
            <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" x="0" y="0" viewBox="0 0 24 24" style="enable-background:new 0 0 512 512" xml:space="preserve" class="lg:w-6 w-7 fill-inherit">
...
<div class="relative" x-data="{sideDropdown: false}">
    ...
    <ul x-show="sideDropdown" class="ms-3 mt-2 flex flex-col gap-2">
        <x-sidebar.sidebar-item href="{{ route('warga.layanan.pengaduan.index') }}" title="Pengaduan" :active="request()->routeIs(['warga.layanan.pengaduan.index'])">
        ...
        <x-sidebar.sidebar-item href="{{ route('warga.layanan.pengaduan.newPengaduanPage') }}" title="Buat Pengaduan" :active="request()->routeIs(['warga.layanan.pengaduan.newPengaduanPage'])">
...
<div class="relative" x-data="{sideDropdown: false}">
    ...
    <ul x-show="sideDropdown" class="ms-3 mt-2 flex flex-col gap-2">
        <x-sidebar.sidebar-item href="{{ route('warga.layanan.reservasiJadwalTemu.index') }}" title="Reservasi" :active="request()->routeIs(['warga.layanan.reservasiJadwalTemu.index'])">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="lg:w-5 w-6 fill-inherit">
        ...
        <x-sidebar.sidebar-item href="{{ route('warga.layanan.reservasiJadwalTemu.newReservasiJadwalTemuPage') }}" title="Buat Reservasi" :active="request()->routeIs(['warga.layanan.reservasiJadwalTemu.newReservasiJadwalTemuPage'])">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="lg:w-5 w-6 fill-inherit">
