@php
    $previewDiv =
        '<div id="' .
        $id .
        '-preview" class="mt-2 rounded-sm bg-cover bg-center bg-no-repeat w-full h-96" style="background-image: url(' .
        $value .
        ')"></div>';
@endphp
...
<div id="{{ $id }}-wrapper" {{ $attributes->merge(['class' => 'mt-4']) }}>
    ...
    <input id="{{ $id }}" placeholder="{{ $placeholder }}" type="file" name="{{ $key }}"
        value="{{ $value }}"
