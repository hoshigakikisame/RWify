<div class="card-container px-3 py-4">
    <div class="card-wrap overflow-hidden rounded-xl border dark:border-gray-700">
        <div class="card-img mx-6 my-5 w-full bg-cover bg-center">
            <img src="{{ $icon }}" alt="" class="h-12 w-12 md:h-16 md:w-16" />
        </div>
        <div class="card-body mx-6 my-5 flex flex-col gap-2">
            <h2 class="font-Poppins text-xl md:text-2xl font-medium leading-6">{{ $title }}</h2>
            <p class="text-xs md:text-md w-60 md:w-80 font-Inter font-light dark:text-gray-400">{{ $description }}</p>
            <div class="action-button">
                <a href="{{ $href }}"
                    {{ $attributes->merge(['class' => ' flex items-center fill-green-800 dark:fill-green-600']) }}>

                    <span class="py-2 text-xs md:text-sm text-green-800 dark:text-green-600">Selengkapnya</span>
