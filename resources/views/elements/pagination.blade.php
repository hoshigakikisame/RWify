...
<div class="flex justify-between items-center mt-4 pt-3">
    <div class="flex">
        <div class="flex items-center">
            ...
            <select name="pageCount" id="pageCount"
                class="bg-white border border-gray-300 dark:bg-gray-900 text-gray-800 dark:text-gray-200 text-sm rounded-lg focus:border-blue-500 focus:ring-blue-400 dark:focus:border-blue-400 dark:focus:ring-blue-400"
    ...
    <div aria-label="pagination">
        <ul class="pagination flex gap-1">
            @if ($paginator->currentPage() > 1)
                <li class="page-item"><button
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1"
                            class="w-5 h-5 rtl:-scale-x-100" stroke-width="1.5" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path
            ...
            @endif
            <li class="page-item"><button id="prev_button"
                    class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800 {{ $paginator->onFirstPage() ? 'cursor-not-allowed' : '' }}"
            ...
            @for ($i = $firstPage; $i <= $lastPage; $i++)
                <li class="page-item"><button {{ $i == $paginator->currentPage() ? 'disabled=1' : '' }}
                        class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800 {{ $i == $paginator->currentPage() ? 'dark:!bg-gray-800 !bg-gray-200 cursor-not-allowed' : '' }}"
                        onclick="paginate({{ $i }})">
                        {{ $i }}
                    </button></li>
            @endfor
            ...
            <li class="page-item"><button id="next_button"
                    class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800 {{ $paginator->onLastPage() ? 'cursor-not-allowed' : '' }}"
...
<script type="module">
    $(document).ready(function() {
        $('#pageCount').val(document.location.search.includes('paginate') ? new URLSearchParams(document
            .location.search).get('paginate') : 5)
    });
</script>
