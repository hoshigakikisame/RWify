        ...
        <div class="flex flex-col flex-col-reverse lg:flex-row gap-8 ">
            <div class="form w-full rounded-lg bg-gray-50 px-8 py-4 pb-8 shadow-sm dark:bg-gray-800/50">
                <form action="{{ route('warga.layanan.reservasiJadwalTemu.new') }}" method="POST" enctype="">
                    ...
                    <div class="form-body">
                        <x-form.input-form title="Judul Reservasi" key="subjek" type="text"
                            placeholder="Masukkan Judul Reservasi" value="{{ old('subjek') }}" />
                        <x-form.textarea-input-form title="Pesan Reservasi" key="pesan" type="text"
                            placeholder="Masukkan Pesan Atau Keterangan Reservasi" value="{{ old('subjek') }}" />
                        <div class="">
                            <x-form.input-form title="Tanggal Reservasi" key="jadwal_temu" type="datetime-local"
                                placeholder="Enter date" />
                            <div class="mt-4">
                                <label for="reservationTargets"
                                    class="block text-sm capitalize text-gray-700 dark:text-gray-300">
                                    Target
                                </label>
                                <select name="nik_penerima" id="reservationTargets"
                        ...
                        <div class="swiper-slide">
                            <div class="flex flex-col rounded-lg border p-7 dark:border-gray-700 w-full">
                                <div class="profile flex gap-2">
                                    <div class="p-12 mr-4 h-20 w-20 rounded-full bg-white bg-cover bg-center dark:bg-gray-900"
                                        style="background-image: url('{{ $user->image_url }}')"></div>
                                    <div class="flex flex-col justify-center">
                                        <p
                                            class="font-Poppins text-md font-medium leading-5 text-gray-800 dark:text-gray-200">
                                            {{ $user->getNamaLengkap() }} - {{ $user->getRole() }}
                                        </p>
