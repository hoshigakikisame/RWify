...
<div class="mt-8 mx-14">
    <div class="relative h-full">
        ...
        <div class="user flex gap-12 relative h-32" x-data="{}">
            <div id="userImage" class="user-avatar relative left-14 -top-20">
                ...
                <form id="userImageChangeForm" action="" method="POST">
                    @csrf
                    <label for="image">
                        <input type="file" aria-current="submitButton" class="hidden" name="image" id="image" accept="image/*" x-effect="window.utils.Request.actionRequest('{{route('user.profile.updateImage')}}','#userImage','#userImageChangeForm',true)">
                        ...
                        <ul id="error" class="z-40 text-sm text-red-600 dark:text-red-400 space-y-1">
                        </ul>
                    </label>
    ...
    <nav class="py-1 pt-0 px-2 mb-2 relative">
        <ul class="flex gap-1 text-sm dark:text-gray-100" x-data="{isPanelActive: false, activeClass:'rounded-t-md text-blue-500 dark:text-blue-400 border-blue-500 border-b-2'}">
            <li :class="isPanelActive ? '' : activeClass" class="py-2 px-4">
                <button @click="appendProfile({{$user}});isPanelActive= !isPanelActive" ariaLabel="Profile">Profile</button>
            </li>
            <li :class="!isPanelActive ? '' : activeClass" class="py-2 px-4">
                <button @click="appendPassword();isPanelActive= !isPanelActive" ariaLabel="changePassword">Change Password</button>
            </li>
        </ul>
        <hr>
    </nav>
