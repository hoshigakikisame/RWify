...
<section class="container px-4 mt-7 mb-8 mx-auto relative" x-data="{ modalOpen: false }">
    <div class=" sm:flex sm:items-center sm:justify-between ">
        ...
        <div class="flex items-center mt-4 gap-x-3" x-data="{ modalOpen: false }">
            <button
                class="flex items-center justify-center md:w-1/2 px-5 py-2 text-sm text-gray-700 transition-colors duration-200 bg-white border rounded-lg gap-x-2 sm:w-auto dark:hover:bg-gray-800 dark:bg-gray-900 hover:bg-gray-100 dark:text-gray-200 dark:border-gray-700">
            ...
            <button id="addButton" @click="modalOpen = !modalOpen"
                class=" flex items-center justify-center text-nowrap md:w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600"
                onclick="request('{{ route('rw.manage.warga.new') }}', '#addModal', '#addModalForm')">
                ...
                <div
                    ...
                    <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300 transform"
                        ...
                        <form class="mt-5" id="addModalForm">
                            ...
                            <x-form.select-input-form title="Jenis Kelamin" key="jenis_kelamin"
                                :options="$genderOptions" placeholder="Pilih Jenis Kelamin Warga" />
    ...
    <div class="mt-6 md:flex md:items-center md:justify-between">
        <div
            ...
            <button id="filter-all" onclick="window.utils.Request.filterRequest({'role': ''})"
                x-effect="let params = new URLSearchParams(window.location.search); (params.has('filters[role]') && params.get('filters[role]') == '') || !params.has('filters[role]') ? $('#filter-all').addClass('!text-blue-400') : $('#filter-all').removeClass('!text-blue-400')"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td class="px-4 py-4 text-sm flex" id="action"
                                        ...
                                        <button id="editButton" @click="modalEditOpen = !modalEditOpen"
                                            ...
                                            type="button"
                                            onclick="(function () {appendUpdateModal({{ $user }},event);request(`{{ route('rw.manage.warga.update') }}`, '#editModal', '#editModalForm')})()">
                                            <span
                                        ...
                                        <button id="deleteButton" @click="modalDeleteOpen = !modalDeleteOpen"
                                            onclick="(function (){appendDeleteModal('{{ $user->getNIK() }}','{{ $user->getNamaDepan() . ' ' . $user->getNamaBelakang() }}',event);request(`{{ route('rw.manage.warga.delete') }}`, '#deleteModal', '#deleteModalForm')})()"
                                            class="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-blue-gray-500 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30">
                                            <span
                                                class="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
