            ...
            <div
                ...
                @foreach (\App\Enums\User\UserRoleEnum::getValues() as $key => $value)
                    @if ($value == \App\Enums\User\UserRoleEnum::WARGA->value)
                        @continue
                    @endif
                    <button id="filter-{{$key}}" onclick="filterRequest('{{ $value }}', event)" x-effect="let params = new URLSearchParams(window.location.search); params.has('role') && params.get('role') == '{{$value}}' ? $('#filter-{{$key}}').addClass('!text-blue-400') : $('#filter-{{$key}}').removeClass('!text-blue-400')"
                        class="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 sm:text-sm dark:hover:bg-gray-800 dark:text-gray-300 hover:bg-gray-100">
                        {{ $value }}
                    </button>
                @endforeach
