...
<section class="container px-4 mt-7 mx-auto relative " x-data="{ modalOpen: false }">
        ...
        <div class="sm:flex sm:items-center sm:justify-between ">
            <div>
                <div class=" flex items-center gap-x-3">
                    ...
                    <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">{{ $propertiInstances->total() }}
                        Properti</span>
                </div>
                @if($propertiInstances->sortByDesc('diperbarui_pada')->first())
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Data ini terakhir diupdate {{ $propertiInstances->sortByDesc('diperbarui_pada')->first()?->getDiperbaruiPada()->diffForHumans(null, true)}} yang lalu</p>
                @else
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Masih belum ada properti yang terdata</p>
                @endif
            ...
            <div class=" md:flex md:items-right md:justify-between">
                <div class="flex items-center mt-2 gap-x-3" x-data="{ modalOpen: false }">
                    <button id="addButton" @click="modalOpen = !modalOpen" class=" flex items-center justify-center mb-2 text-nowrap px-5 py-2.5 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600" onclick="window.utils.Request.actionRequest('{{ route('rw.manage.properti.new') }}', '#addModal', '#addModalForm')">
                        ...
                        <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
                            ...
                            <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-xl p-8 my-20 overflow-hidden text-left transition-all transform bg-white dark:bg-gray-800 rounded-lg shadow-xl 2xl:max-w-2xl">
                                <div class="flex items-center justify-between space-x-4">
                                    <h1 class="text-xl font-medium text-gray-800 dark:text-gray-100 ">Tambah Properti</h1>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-4 pe-4 pe-0 ps-6 text-sm flex" id="action" x-data="{ modalEditOpen: false, modalDeleteOpen: false }">
                                    ...
                                    <button id="editButton" @click="modalEditOpen = !modalEditOpen" class="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-blue-gray-500 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30" type="button" onclick="(function () {appendUpdateModal({{ $properti }},event);window.utils.Request.actionRequest(`{{ route('rw.manage.properti.update') }}`, '#editModal', '#editModalForm')})()">
    ...
    {{ $propertiInstances->onEachSide(-1)->links('elements.pagination') }}
