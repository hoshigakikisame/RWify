                        ...
                        <div class="card pt-4 border border-gray-200 ring-4 shadow-sm ring-gray-200/10 dark:border-gray-800 dark:ring-gray-700/10 rounded-lg overflow-hidden">
                            <div class="card-heading mb-1 px-8 flex justify-between items-start">
                                ...
                                <div class="action-dropdown relative" x-data="{ actionModalOpen: false }">
                                    ...
                                    <div class="absolute bg-white right-8 top-0 rounded-lg border dark:border-gray-700 overflow-hidden divide-y dark:divide-gray-700" style="display: none;" x-show="actionModalOpen" @click.away="actionModalOpen = false" x-data="{ modalEditOpen: false, modalDeleteOpen: false, modalShareOpen:false }">
                                        <div class=" relative">
                                            <a id="detailButton" class="inline-flex gap-2 px-4 pt-3 pb-2  hover:bg-gray-100 dark:hover:bg-gray-800 w-full items-center" target="_blank" href="{{ route('informasi.pengumuman.detail', [$pengumuman->getIdPengumuman()]) }}">
                                                ...
                                                <p class="font-medium text-xs text-gray-600 dark:text-gray-400 me-4">
                                                    Detail</p>

                                            </a>
                                        </div>
                                        <button id="shareButton" class="inline-flex gap-2 px-4 pt-2 pb-2  hover:bg-gray-100 dark:hover:bg-gray-800 w-full items-center" @click="modalShareOpen = !modalShareOpen" type="button" onclick="(function () {appendShareModal('{{ $pengumuman->getIdPengumuman() }}','{{ $pengumuman->getJudul() }}','{{ $pengumuman->getStatus() }}',event);window.utils.Request.actionRequest(`{{ route('rw.manage.pengumuman.changeStatus') }}`, '#shareModal', '#shareModalForm')})()" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                                            <div class="icon w-3 fill-green-500">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                                    @if ($pengumuman->getStatus() == 'publish')
                                                    <path d="M16 12v2c0 1.103-.897 2-2 2h-4c-1.103 0-2-.897-2-2v-2H0v9c0 1.654 1.346 3 3 3h18c1.654 0 3-1.346 3-3v-9zm6 9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-7h4c0 2.206 1.794 4 4 4h4c2.206 0 4-1.794 4-4h4zM10.586 10.414 7.293 7.121l1.414-1.414L11 8V0h2v8l2.293-2.293 1.414 1.414-3.293 3.293c-.39.39-.902.585-1.414.585s-1.024-.195-1.414-.585" />
                                                    @else
                                                    <path d="M16 12v2c0 1.103-.897 2-2 2h-4c-1.103 0-2-.897-2-2v-2H0v9c0 1.654 1.346 3 3 3h18c1.654 0 3-1.346 3-3v-9zm6 9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-7h4c0 2.206 1.794 4 4 4h4c2.206 0 4-1.794 4-4h4zM8.707 5.293 7.293 3.879 10.586.586a2.003 2.003 0 0 1 2.828 0l3.293 3.293-1.414 1.414L13 3v8h-2V3z" />
                                                </svg>
                                                @endif
                                            </div>
                                            <p class="font-medium text-xs text-green-600 dark:text-green-400 me-4">
                                                @if ($pengumuman->getStatus() == 'publish')
                                                Draf
                                                @else
                                                Publish
                                        ...
                                        <button id="editButton" class="inline-flex gap-2 px-4 pt-2 pb-2  hover:bg-gray-100 dark:hover:bg-gray-800 w-full items-center" @click="modalEditOpen = !modalEditOpen" type="button" onclick="(function () {appendUpdateModal({{ $pengumuman }},event);window.utils.Request.actionRequest(`{{ route('rw.manage.pengumuman.update') }}`, '#editModal', '#editModalForm',true)})()" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                            ...
                            <div class="card-footer bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-800 px-8 py-2 flex justify-between">
