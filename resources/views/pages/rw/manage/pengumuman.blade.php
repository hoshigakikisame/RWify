                ...
                <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
                    ...
                    <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-xl p-8 my-20 overflow-hidden text-left transition-all transform bg-white dark:bg-gray-800 rounded-lg shadow-xl 2xl:max-w-2xl">
                        ...
                        <form class="mt-5" id="addModalForm">
                            @csrf
                            <x-form.inputform title="Judul Pengumuman" key="judul" type="text" placeholder="Gotong Royong Warga" class="col-span-2" />
                            <x-form.textareainputform title="Konten" key="konten" placeholder="dimas anjay mabar lagi main bersama bocah silkroach" />
                    ...
                    <div class="Wraper flex flex-col gap-4">
                        ...
                        <div class="card pt-4 border border-gray-200 ring-4 shadow-sm ring-gray-200/10 dark:border-gray-800 dark:ring-gray-700/10 rounded-lg overflow-hidden">
                                ...
                                <div class="action-dropdown relative" x-data="{actionModalOpen:false }">
                                    ...
                                    <div class="absolute right-8 top-0 rounded-lg border dark:border-gray-700 overflow-hidden divide-y dark:divide-gray-700" style="display: none;" x-show="actionModalOpen" @click.away="actionModalOpen = false">
                                        <button class="inline-flex gap-2 px-4 pt-3 pb-2  hover:bg-gray-100 dark:hover:bg-gray-800 w-full items-center" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                                            ...
                                            <p class="font-medium text-xs text-blue-600 dark:text-blue-400 me-4">Edit</p>
                                        </button>
                                        <button class="inline-flex gap-2 px-4 pb-3 pt-2 hover:bg-gray-100 dark:hover:bg-gray-800 w-full border-b-2 items-center">
                                            ...
                                            <p class="font-medium text-xs text-rose-500 dark:text-rosse-400 me-4">Delete</p>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-800 px-8 py-2 flex justify-between">
                                <p class="text-xs text-gray-300 dark:text-gray-600">Created at {{date('d/m/y', strtotime($pengumuman->getDibuatPada()));}}</p>
                                <p class="text-xs text-gray-400 dark:text-gray-500">Last Updated at {{date('d/m/y H:i', strtotime($pengumuman->getDiperbaruiPada()));}}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
